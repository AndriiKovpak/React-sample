...
import { useCreateFirmMutation } from "../../graphql/urql-codegen/code";
...
interface ICreateFirm {
  name: string;
}

const firmValidationSchema: yup.ObjectSchema<ICreateFirm> = yup.object({
  name: yup.string().min(1).required(),
});

export const CreateFirmModal: FC<CreateModalProps> = ({ isOpen, closeFn }) => {
  ...
  const { errors, touched, isSubmitting, handleChange, handleBlur, handleSubmit } = useFormik({
    ...
    onSubmit: async (args) => {
      const { error } = await createFirm(args);
      if (!error) {
    ...
    >
      <div className="relative flex h-auto w-full flex-col rounded-lg border-0 bg-black shadow-lg outline-none focus:outline-none">
        <div className="flex items-center justify-between rounded-t p-8">
          <h1 className="text-3xl font-semibold text-white">Create Firm</h1>
